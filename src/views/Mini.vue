<template>
    <div>
        <div  class="container" v-if="grid.length && ! modal.show">
            <div class="main">
                <div class="left">
                <p class="nav">{{ subject.area.title }}
                    <i class="fa fa-angle-right" aria-hidden="true"></i> {{ subject.title }}
                </p>
                <div class="progress">
                    ...
                    <h1 class="expression" id="mathjax-wrappper" v-if="grid[row][col].type == 3" v-html="grid[row][col].description"></h1>
                    <h1 v-else>
        ...
        <FinishMiniTest v-if="modal.show" @goDashboard="goDashboard" />
    </div>
</template> 
<script>
    import api from '../api'
    import FinishMiniTest from '../components/FinishMiniTest'
    ...
    export default {
        ...
        methods: {
            goDashboard() {
                this.$router.push("/subject/" + this.mini.subject.slug)
            },
            ...
            correct() {      
                ...
                api.createMiniResult(this.user.id, {
                    mini_id: this.mini.id,
                    status: 'complete',
                    problem_status: 'success',
                    timer: this.timer.minutes * 60 + this.timer.seconds
                })

                api.addPointMiniTest({
                    user: this.user.id,
                    mini: this.mini.id,
                    points: this.grid[this.row][this.col].score
                })
            ...
            renderMathJax(desc) {
                setTimeout(() => {
                    window.MathJax.Hub.Queue(["Typeset", window.MathJax.Hub, document.getElementById('mathjax-wrappper')])
                }, 0)
            },
            wrong() {
                this.renderMathJax()
